...
onMounted(() => {
  ...
  homeControl.onAdd = function (map) {
      ...
      controlDiv.style.backgroundImage = "url('https://img.icons8.com/material-rounded/24/home.png')";
      ...
      controlDiv.onclick = function(){
        ...
        map.setView([em.latitude, em.longitude], 8);
      }
...
watch(() => props.points, (newPoints, oldPoints) => {
    ...
    L.marker([em.latitude, em.longitude], {title: "Emergencia", icon: greenIcon}).addTo(markersLayer).bindPopup(`<b>${em.name}</b><br>${em.description}`);
